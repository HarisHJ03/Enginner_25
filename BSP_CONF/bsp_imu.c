...
#define MPU_INIT_DELAY(x) delay_ms(x)
...
#if 1
  mpu_data.az   = (mpu_buff[0] << 8 | mpu_buff[1]);
  mpu_data.ay   = (mpu_buff[2] << 8 | mpu_buff[3]);
  mpu_data.ax   = -(mpu_buff[4] << 8 | mpu_buff[5]);
  mpu_data.temp = mpu_buff[6] << 8 | mpu_buff[7];

  mpu_data.gz = ((mpu_buff[8] << 8 | mpu_buff[9])   - mpu_data.gx_offset);
  mpu_data.gy = ((mpu_buff[10] << 8 | mpu_buff[11]) - mpu_data.gy_offset);
  mpu_data.gx = -((mpu_buff[12] << 8 | mpu_buff[13]) - mpu_data.gz_offset);
